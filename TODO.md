...
- [x] ⏰ **World Cup — final state Jul 12 evening** (Raymond reopened for the games, reversing the same-day archival): "Watch Parties" parchment panel leads the What's On Now tab — France v Spain (Tue, bar & snacks 3–5 only), England v Argentina (Wed, bar & snacks 3–5 + dinner), The Final (Sun, bar & snacks 3–5); no 3rd-place game; hero banner enlarged. REMAINING → task `luca-worldcup-banner-timeout` (Mon Jul 20 9 AM, pre-authorized): remove hero banner + its CSS, archive Watch Parties to Missed Events with results, strip leftover specialOpeningHours, deploy
