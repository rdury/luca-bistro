...
**Frédéric queue: empty** (Jul 12 — Raymond deployed the former bundle as owner). Live now: `#private-events` section (INTERIM: inquiries via mailto to events@lucabistro.com — Raymond confirmed the address receives mail; the real request form arrives with the dury.net admin project, then the mailto gets replaced) and the "Cadre Bistro" parchment menu (CSS-only: every menu tab on aged parchment + grain in the gilt frame with faint cicada watermark; 4-look mockup kept locally as untracked `menu-parchment-preview.html`). Photos for the private-events section still owed by Raymond. Both `archive/*.patch` files are superseded by this deploy — delete after Raymond's OK.
